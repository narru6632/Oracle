...
begin
    ...
    */
    if SQL%notfound then
        ...
        dbms_output.put_line(SQL%rowcount ||'명의 자료가 수정됬습니다.');
...
-- 함수는 반드시 반환타입을 명시해야 한다. 성병 판단 후 '남자' 혹은 '여자'를
-- 반환하므로 문자형으로 선언한다.
return varchar2
is
...
select findGender('010909-3000000') from dual;
select findGender('010919-4000000') from dual;
...
-- 테입르의 스키마(구조)만 복사한다. where절에 거짓의 조건을 주면 레코드는
-- select되지 않는다.
...
create or replace trigger trig_dept_backup
    ...
    /*
        ...
        수행된다. 만약 문자(테이블)단위 트리거로 정의하고 깊다면 해당
...
-- 입려된다. 행단위 크리거는 적용된 행의 갯수만큼 실행된다.
select * from trigger_dept_original;
select * from trigger_dept_backup;
