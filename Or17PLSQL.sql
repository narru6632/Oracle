...
PL/SQL procedural language
    ...
    DB업무를 처리하기위해 최적화된 언어이다.
*/
-- 예제1] PL/SQL 맛보기

set serveroutput on;
-- ㄴ 주석붙이면 실행 안됨
-- 화면상의 내용을 출력하고 싶을 때 on으로 설정
-- off일때는 출력되지 않는다(실행자체는 됨)

declare              -- 선언부 : 주로 변수를 선언한다
    cnt number;      -- 주로 숫자타입의 변수를 선언
begin                -- 실행부 : begin ~ end절 사이에 실행을 위한 로직을 기술한다.
    cnt := 10;        -- 변수에 10을 대입한다. 대입연산자는 :=를 사용한다.
    cnt := cnt + 1;
    dbms_output.put_line(cnt); --자바의 prinln
end;
/

/*
    PL/SQL문장의 끝에는 /를 붙여야하는데, 만약 없으면
    호스트환경으로 빠져나오지 못한다, 즉 PL/SQL 문장을 위해 필요하다
...
begin
    ...
    실행부 : select절에서 가져온 결과를 선언부에서 선언한 변수에
        1:1 로 대입하여 값을 저장한다. 이때 into를 사용한다.
    */
    select
...
복합변수
    ...
    복합변수 자료형을 만들때에는 일반변수와 참조변수 2가지를 복합해서 
    사용할 수 있다.
*/
/*
...
declare
    ...
    /*
    앞에서 선언한 복합변수 자료형을 통해 선언한 복합변수로 
    3개의 값을 저장할 수 있다.
    */
    record3 emp_3type;
begin
...
바인드 변수
    ...
    호스트환경이란 -> PL/SQL 블럭을 제외한 나머지 부분을 말한다
...
/
--호스트 환경에서 출력시에는 print를 사용 한다
--만약 출력이 안된다면 cmd에서 확인해보면 정상적으로 출력이된다
print return_var;
...
-- 퀴즈] 위 예제를 치환연산자를 통해 숫자를 입력받을 수 있도록 수정하시오
...
begin
    ...
    from employees
    where employee_id = emp_id; -- 입력받은 사원 번호를 쿼리에 적용한다.
...
begin
   ...
   /*
    case~when문이 if문과 다른 점은 할당할 변수를 먼저 선언한 후 문장내에서
...
/*
반복문 1 : Basic Loop
    Java의 do-while문과 같이 조건체크 없이 loop진입한 후 탈출조건이 될때까지
...
begin
    ...
    -- 0~10까지 출력한다
        ...
        -- exit는 java의 break;와 동일한 역할을 한다
        end loop;
    end;
/
--예제11] 제어문(반복문 : basic loop)
/*
시나리오] Basic loop문으로 1부터 10까지의 정수의 
    합을 구하는 프로그램을 작성하시오.
*/
...
begin
    ...
    dbms_output.put_line('1~10 까지의 합은: ' || sumNum);
end;
/
...
반복문2 : while문
    ...
    즉, 조건에 맞지 않는다면 한번도 실해되지 않을 수 있다.
...
/*
예제15] 제어문(반복문 : for) 
    : 반복의 횟수를 지정하여 사용할 수 있는 반복문으로, 반복을 위한 변수를 별도로
    선언하지 않아도 된다. 그러므로 특별한 이유가 없으면 
...
--줄바꿈 있는 버전
begin 
    for dan in 2 .. 9 loop -- 2~9 반복
        dbms_output.put_line(dan||'단'); -- 단 라벨 부착
        for su in 1 .. 9 loop -- 1~9 반복
            dbms_output.put_line(dan||'*'||su||'='||(dan*su)); -- 구구단을 즉시출력 + 줄바꿈
        end loop;
    end loop;
end;
/
...
--줄바꿈 안되는 버전
declare
    -- 구구단에서 하나의 단을 저장하기 위한 변수를 지정
    guguStr varChar2(1000);
begin
    for dan in 2 .. 9 loop -- 단에 해당하는 루프
        for su in 1 .. 9 loop -- 수에 해당하는 루프
            --하나의 단을 문자형 변수에 누적해서 연결
            guguStr := guguStr || dan || '*' || su || '=' || (dan*su) || ' ';
        end loop;
        -- 하나의 단만큼 누적이 완료되면 즉시 출력한다.
        dbms_output.put_line(guguStr);
        -- 그 다음 단을 다시 저장하기 위해 초기화
        guguStr :=  '';
    end loop;
end;
/
...
커서(Cursor)
    ...
    Open Cursor
        :쿼리를 수행하라는 의미. 즉 Open 할때 Cursor 선언시에 select문장이
    ...
    Close Cursor
        : 커서 닫기로 결과셋이 자원으로 반납한다. select 문장이 모두 처리된
        후 Cursor를 닫아준다.
        
    Cursor의 속성
        &Found : 가장 최근에 인출(Fetch)이 행을 return하면 True, 아니면 False를 반환한다.
        &NotFound: % Found의 반대값을 반환한다.
...
declare
    ...
    /* 커서 선언 : 부서 테이블의 모든 레코드를 조회하는 SELECT문으로 INTO절이 없는 형태로 기술한다
        쿼리의 실행결과는 CUR1에 저장된다 */
    cursor cur1 is
        select
...
begin
    open cur1;
    /*
    해당 쿼리문을 수행해서 결과셋을 가져온다. 결과셋이란 쿼리(질의)문을 수행한
    후 반환되는 레코드의 결과를 말한다.
    */
    
    loop
        -- fetch한 결과는 참조변수에 각 각 저장한다.
        fetch cur1 into
...
-- 예제 18] Cursor
/*
시나리오] Cursor을 사용하여 사우너테이블에서 커미션이 null이 아닌 
...
1. 연관배열(Associative Array)
     ...
     Key : 자료형은 주로 숫자를 사용한다. binary_integer, pls_integer를 주로 
...
declare
    Type avType Is
        Table Of varchar2(30) --값의 자료형 선언
        Index By varchar2(10); --키, 인덱스의 자료형 선언
    -- 연관배열 타입의 변수를 선언
    var_array avType;
begin
    -- 연관배열 값 할당
